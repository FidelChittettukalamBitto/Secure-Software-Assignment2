...
unsigned char inv_s[256] = { 0x52, 0x09, 0x6A, 0xD5, 0x30, 0x36, 0xA5, 0x38, 0xBF, 0x40, 0xA3, 0x9E, 0x81, 0xF3, 0xD7, 0xFB,
...
unsigned char rcon[BLOCK_SIZE] = { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a
	/*
...
void shift_rows(unsigned char *block) {
    ...
    for (int i = 0; i < BLOCK_SIZE; i += 4) {
        ...
        temp_block[(i + 1) % BLOCK_SIZE] = block[(i + 1) % BLOCK_SIZE];
        temp_block[(i + 2) % BLOCK_SIZE] = block[(i + 2) % BLOCK_SIZE];
        temp_block[(i + 3) % BLOCK_SIZE] = block[(i + 3) % BLOCK_SIZE];
    }
    for (int i = 0; i < BLOCK_SIZE; i++) {
...
void invert_shift_rows(unsigned char *block) {
    ...
    for (int i = 0; i < BLOCK_SIZE; i += 4) {
        temp_block[i] = block[i];
        temp_block[(i + 1) % BLOCK_SIZE] = block[(i + 13) % BLOCK_SIZE];
        temp_block[(i + 2) % BLOCK_SIZE] = block[(i + 10) % BLOCK_SIZE];
        temp_block[(i + 3) % BLOCK_SIZE] = block[(i + 7) % BLOCK_SIZE];
    }
    for (int i = 0; i < BLOCK_SIZE; i++) {
        block[i] = temp_block[i];
    }
...
void aes_encrypt_block(unsigned char *block, unsigned char *cipher_key) {
    ...
    for (int round = 1; round < AES_ROUNDS; round++) {
        ...
        shift_rows(block);
        mix_columns(block);
        add_round_key(block, round_keys + round * AES_BLOCK_SIZE);
    }
